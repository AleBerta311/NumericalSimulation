...
private:
  ...
  vector<double> _block_en;         // individual energies in block , needed for cv  WIP
  vector<double> _block_fluc;      //sigma in each block, needed for cv WIP
  vector<double> cv;               // yessir
  vector<double> gofr2;            // need to accumulate the square of averages of g(r) after each block calc.
