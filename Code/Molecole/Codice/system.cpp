...
void System :: move(int i){ // Propose a MC move for particle i
  ...
  { 
    ...
    p_pos = 1/(1+exp(-_beta*deltaE));    //chance that s(i) = +1 at the next step 

    if (_rnd.Rannyu(0,1) < p_pos)
      _particle(i).setspin( +1 );      //sets it to +1 regardless of current
    else
      _particle(i).setspin(-1);       // sets spin to -1 regardless of current
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){
    ...
    if( property == "SIMULATION_TYPE" ){
      ...
      break;
    } else cerr << "PROBLEM: unknown input property: " << property << " " << endl;
...
void System :: initialize_properties(){ // Initialize data members used for measurement of properties
  ...
  ifstream input("/home/laledudu/Code/Molecole/INPUT/properties.dat");
  if (input.is_open()){
    while ( !input.eof() ){
      input >> property;
      if( property == "POTENTIAL_ENERGY" ){
        ofstream coutp("/home/laledudu/Code/Molecole/OUTPUT/potential_energy.dat");
        ...
        index_property++;
        _vtail = 8*PI*_rho/3.0 *(1/(3*pow(_r_cut, 9)) - 1/(pow(_r_cut,3)));       // Per <V>_tail /N
      } else if( property == "KINETIC_ENERGY" ){
        ofstream coutk("/home/laledudu/Code/Molecole/OUTPUT/kinetic_energy.dat");
        ...
        ofstream coutt("/home/laledudu/Code/Molecole/OUTPUT/total_energy.dat");
        ...
        ofstream coutte("/home/laledudu/Code/Molecole/OUTPUT/temperature.dat");
        ...
        ofstream coutpr("/home/laledudu/Code/Molecole/OUTPUT/pressure.dat");
        ...
        index_property++;
        _ptail = 32* PI* _rho * ( 1/(9*pow(_r_cut,9))  -1/(6*pow(_r_cut, 3))); 
      } else if( property == "GOFR" ){
        ofstream coutgr("/home/laledudu/Code/Molecole/OUTPUT/gofr.dat");
        coutgr << "# DISTANCE:     AVE_GOFR:     GLOBAL AV_GOFR:     ERROR:" << endl;
        coutgr.close();
        input>>_n_bins;
        _nprop+=_n_bins;
        ...
        ofstream coutpr("/home/laledudu/Code/Molecole/OUTPUT/magnetization.dat");
        ...
        ofstream coutpr("/home/laledudu/Code/Molecole/OUTPUT/specific_heat.dat");
        ...
        ofstream coutpr("/home/laledudu/Code/Molecole/OUTPUT/susceptibility.dat");
        ...
        ofstream coutf;
        coutf.open("/home/laledudu/Code/Molecole/OUTPUT/output.dat",ios::app);
  ...
  /*_block_en.resize(_nsteps);      // WIP
  _block_fluc.resize(_nblocks);     //WIP   dont use with std:: vector pushback
  */
...
void System :: write_configuration(){
  ofstream coutf;
  if(_sim_type < 2){
    coutf.open("/home/laledudu/Code/Molecole/OUTPUT/CONFIG/config.xyz");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    coutf.open("/home/laledudu/Code/Molecole/OUTPUT/CONFIG/config.spin");
...
void System :: write_XYZ(int nconf){
  ofstream coutf;
  coutf.open("/home/laledudu/Code/Molecole/OUTPUT/CONFIG/config_" + to_string(nconf) + ".xyz");
  if(coutf.is_open()){
    coutf << _npart << endl;
    coutf << "#Comment!" << endl;
    for(int i=0; i<_npart; i++){
...
void System :: write_velocities(){
  ofstream coutf;
  coutf.open("/home/laledudu/Code/Molecole/OUTPUT/CONFIG/velocities.out");
  if(coutf.is_open()){
    for(int i=0; i<_npart; i++){
...
void System :: read_configuration(){
  ifstream cinf;
  cinf.open("/home/laledudu/Code/Molecole/INPUT/CONFIG/config.xyz");
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
  ...
  if(_restart and _sim_type > 1){
    int spin;
    cinf.open("/home/laledudu/Code/Molecole/INPUT/CONFIG/config.spin");
    for(int i=0; i<_npart; i++){
      cinf >> spin;
...
void System :: measure(){ // Measure properties
      ...
      for (int j=i+1; j<_npart; j++){
        ...
        if(dr < _r_cut)
        {
          ...
          if(_measure_pressure)
            virial+= 48 * (pow(1/dr,12) -0.5*pow(1/dr,6));    // PRESSURE  
        }
      }
    }
  }
  ...
  if(_measure_pressure)
  {
    virial = 1/(3*_volume) * virial/double(_npart) +_ptail/_volume;
    double temp_estim = (2.0/3.0) * kenergy_temp;
    _measurement(_index_pressure) = _rho* temp_estim + virial;      // virial th. and natural units
  }
  ...
  if(_measure_gofr)
  {
    ...
    {
      double r = _bin_size*i;
      double delta_V = 4*PI/3 * (pow(r +_bin_size,3) -pow(r,3)); 
      double norm = _rho*_npart*delta_V;    //normalisation factor for each bin
      gofr(i)/=norm;
    }

    for (int i = 0; i<_n_bins; i++)
      _measurement(_index_gofr+i) = gofr[i];
  ...
  if (_measure_magnet)
  {
    ...
    for (int j = 0; j< _npart; j++)
    {
      magn+= _particle(j).getspin();
    }
    _measurement(_index_magnet) = (double)magn/_npart;           // It's PER PARTICLE
  }
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////

  if (_measure_cv)
  {
    _block_en.push_back (tenergy_temp);
  ...
  if(_measure_chi)
  {
    ...
    for (int j = 0; j< _npart; j++)
    {
      _magnet+= _particle(j).getspin();
    }

    _magnet= pow(_magnet, 2);

    _magnet*= _beta;
    _measurement(_index_chi) = _magnet/_npart;      // It's calculated PER PARTICLE
  }
...
void System :: averages(int blk){
  ...
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    coutf.open("/home/laledudu/Code/Molecole/OUTPUT/potential_energy.dat",ios::app);
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
    coutf.open("/home/laledudu/Code/Molecole/OUTPUT/kinetic_energy.dat",ios::app);
  ...
  // TOTAL ENERGY //////////////////////////////////////////////////////////////
  if (_measure_tenergy){
    coutf.open("/home/laledudu/Code/Molecole/OUTPUT/total_energy.dat",ios::app);
  ...
  // TEMPERATURE ///////////////////////////////////////////////////////////////
  if (_measure_temp){
    coutf.open("/home/laledudu/Code/Molecole/OUTPUT/temperature.dat",ios::app);
  ...
  if (_measure_pressure)
  {
    coutf.open("/home/laledudu/Code/Molecole/OUTPUT/pressure.dat", ios::app);
  ...
  if (_measure_gofr)
  {
    coutf.open("/home/laledudu/Code/Molecole/OUTPUT/gofr.dat", ios::app); 

    double accum = 0, global_accum = 0; 

    for (int i = 0; i< _n_bins; i++)
    {
      accum+= _average(_index_gofr+i);   //increase the accumulator with the block-average for the i-th bin
      global_accum+= _global_av(_index_gofr+i);
    }
    
    accum/=_n_bins;     // This is the average value of g(r) in the block
    global_accum/=_n_bins;
    
    gofr2.push_back(accum*accum); 
    sum_ave2 = 0;
    for (int j = 0; j< blk; j++)
      sum_ave2+=gofr2[j];
    ...
    coutf << setw(12) << blk
          << setw(12) << accum
          << setw(12) << global_accum/double(blk)
          << setw(12) << this->error(global_accum, sum_ave2, blk) << endl;

    coutf.close();

    coutf.open("/home/laledudu/Code/Molecole/OUTPUT/gofr_histo.dat");
    for (int j = 0; j< _n_bins; j++)
    {
      coutf << setw(12) << (j*_bin_size +(j+1)*_bin_size)/2
            << setw(12) << _global_av(_index_gofr+j) << endl;
    }
  ...
  if (_measure_magnet)
  {
    coutf.open("/home/laledudu/Code/Molecole/OUTPUT/magnetization.dat", ios::app);
  ...
  if (_measure_cv)
  {
    coutf.open("/home/laledudu/Code/Molecole/OUTPUT/specific_heat.dat", ios::app);

    double sigma = 0;

    for (int i = 0; i<_nsteps; i++)
      sigma+= pow (_average(_index_tenergy) - _block_en[i] , 2);
    
    sigma/=(_nsteps-1);


    _block_fluc.push_back(sigma);
    double prog_cv = 0;

    for (int j = 0; j< blk; j++)
      cv[blk-1]+= _block_fluc[j];
    
    cv[blk-1]/=blk;

    cv[blk-1] = _beta*_beta*cv[blk-1]*_npart;


    // Now i gotta do more stuff to get the global average and errors smh 
    double glob_cv  = 0;
    double glob_cv2 = 0;
    for (int j = 0; j< blk; j++)
    {
    ...
    double error = sqrt ( (glob_cv2- pow(glob_cv,2))  /double(blk));
  ...
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
   ...
   if (_measure_chi)
  {
    coutf.open("/home/laledudu/Code/Molecole/OUTPUT/susceptibility.dat", ios::app);
  ...
  // ACCEPTANCE ////////////////////////////////////////////////////////////////
  double fraction;
  coutf.open("/home/laledudu/Code/Molecole/OUTPUT/acceptance.dat",ios::app);
