...
void Population::Remove_last_individual()
{
    if (!m_pop.empty()) // Check if the population vector is not empty
    {
        m_pop.pop_back(); // Remove the last element
        m_pop_id.pop_back(); // Also remove the corresponding ID
        m_pop_count--; // Decrease the population count
    }
}
