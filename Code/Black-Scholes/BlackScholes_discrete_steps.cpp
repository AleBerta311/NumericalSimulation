...
double EvaluateFinalPrice(double, double , double, double, Random&, int);


int main()
{
    ofstream myoutput;
    myoutput.open("call_prices_discr.txt");
    ...
    for (int j = 0; j< n_eval/n_in_block; j++)
    {
        ...
        {
            final_asset_price = EvaluateFinalPrice(initial_asset_price, delivery_time, no_risk_interest_rate, volatility, myGen, n_steps);
    ...
    myoutput.open("put_prices_discr.txt");
...
double EvaluateFinalPrice(double S_init, double final_T, double mu, double sigma, Random& myGen, int nsteps)
{
    double Z;      //Z of the GBM

    double final_price = S_init;

    double deltaT = final_T/nsteps;

    for (int i = 0; i< nsteps; i++)
    {
        Z = myGen.Gauss(0, 1); 
        final_price= final_price*exp((mu- 0.5*sigma*sigma)*deltaT+sigma*Z*sqrt(deltaT));
    }
